...
class ReportController {
    ...
    static async financial(req, res) {
        try {
         ...
         ORDER BY date ASC`, [startDate, endDate]);
            ...
            const categoryResult = await database_1.default.query(`SELECT 
         ...
         ORDER BY total DESC`, [startDate, endDate]);
            // Totais
            const totalsResult = await database_1.default.query(`SELECT 
          ...
          (SELECT COALESCE(SUM(amount), 0) FROM accounts_payable WHERE status = 'paid' AND payment_date >= $1 AND payment_date <= $2) as total_expense`, [startDate, endDate]);
            res.json({
    ...
    static async sales(req, res) {
        try {
            ...
            const endDate = req.query.end_date ? new Date(req.query.end_date) : new Date();
            // Vendas por dia
            const dailyResult = await database_1.default.query(`SELECT 
         ...
         ORDER BY date ASC`, [startDate, endDate]);
            // Vendas por mecânico
            const mechanicResult = await database_1.default.query(`SELECT 
         ...
         ORDER BY total DESC`, [startDate, endDate]);
            // Vendas por cliente
            const clientResult = await database_1.default.query(`SELECT 
         ...
         LIMIT 10`, [startDate, endDate]);
            // Status das OS
            const statusResult = await database_1.default.query(`SELECT 
          status,
          COUNT(*) as count,
          SUM(total) as total
         FROM orders
    ...
    static async inventory(req, res) {
        try {
            ...
            const lowStockResult = await database_1.default.query(`SELECT 
          p.*,
         ...
         ORDER BY (p.current_quantity - p.min_quantity) ASC`);
            ...
            const movementsResult = await database_1.default.query(`SELECT 
          im.*,
         ...
         ORDER BY total_value DESC`);
            ...
            const totalValueResult = await database_1.default.query(`SELECT 
    ...
    static async clients(req, res) {
        try {
            const startDate = req.query.start_date ? new Date(req.query.start_date) : new Date(new Date().getFullYear(), 0, 1);
            const endDate = req.query.end_date ? new Date(req.query.end_date) : new Date();
         ...
         LIMIT 20`, [startDate, endDate]);
            // Clientes por tipo
            const typeResult = await database_1.default.query(`SELECT 
          type,
          COUNT(*) as count
         FROM clients
         WHERE active = true
         GROUP BY type`);
            // Novos clientes
            const newClientsResult = await database_1.default.query(`SELECT 
    ...
    static async overview(req, res) {
        try {
          ...
          (SELECT COUNT(*) FROM accounts_payable WHERE status = 'open' AND due_date < CURRENT_DATE) as overdue_payables`, [startDate, endDate]);
            res.json({
                period: { start: startDate, end: endDate },
