...
exports.saveSignatureValidation = exports.updateItemValidation = exports.addItemValidation = exports.createOrderValidation = exports.OrderController = void 0;
const order_model_1 = require("../models/order.model");
const product_model_1 = require("../models/product.model");
const inventory_movement_model_1 = require("../models/inventory-movement.model");
const order_history_model_1 = require("../models/order-history.model");
...
const order_file_model_1 = require("../models/order-file.model");
...
class OrderController {
    static async list(req, res) {
        try {
            ...
            const orders = await order_model_1.OrderModel.findAll(status, clientId, mechanicId, search);
            res.json(orders);
        }
        catch (error) {
    ...
    static async getById(req, res) {
        try {
            ...
            const items = await order_model_1.OrderModel.getItems(id);
            const history = await order_history_model_1.OrderHistoryModel.findByOrder(id);
            const files = await order_file_model_1.OrderFileModel.findByOrder(id);
    ...
    static async create(req, res) {
        try {
            ...
            if (template_id) {
                const template = await order_template_model_1.OrderTemplateModel.findById(parseInt(template_id));
                if (template && template.items && template.items.length > 0) {
                    ...
                    await order_model_1.OrderModel.updateTotals(order.id);
                    // Retornar ordem atualizada com itens
                    const updatedOrder = await order_model_1.OrderModel.findById(order.id);
                    const items = await order_model_1.OrderModel.getItems(order.id);
    ...
    static async update(req, res) {
        try {
            ...
            const existingOrder = await order_model_1.OrderModel.findById(id);
            if (!existingOrder) {
                res.status(404).json({ error: 'Ordem de serviço não encontrada' });
                return;
            }
            const order = await order_model_1.OrderModel.update(id, req.body);
            // Se mudou status para finished, atualizar finished_at
            if (req.body.status === 'finished' && !order.finished_at) {
                await order_model_1.OrderModel.update(id, { finished_at: new Date() });
            }
            // Se mudou status para in_progress, atualizar started_at
            if (req.body.status === 'in_progress' && !order.started_at) {
                await order_model_1.OrderModel.update(id, { started_at: new Date() });
            }
            const updatedOrder = await order_model_1.OrderModel.findById(id);
    ...
    static async delete(req, res) {
        try {
            ...
            try {
                const receivables = await account_receivable_model_1.AccountReceivableModel.findAll(undefined, undefined, undefined, undefined);
                const linkedReceivables = receivables.filter((ar) => ar.order_id === id);
                if (linkedReceivables.length > 0) {
                    // Verificar se as contas estão pagas ou podem ser canceladas
                    const unpaidReceivables = linkedReceivables.filter((ar) => ar.status === 'open' || ar.status === 'overdue');
                    if (unpaidReceivables.length > 0) {
                        res.status(400).json({
                            ...
                            receivable_ids: unpaidReceivables.map((ar) => ar.id)
                        });
                        return;
                    }
                    ...
                    const updateClient = await database_1.default.connect();
                    try {
                        for (const receivable of linkedReceivables) {
                            await updateClient.query('UPDATE accounts_receivable SET order_id = NULL WHERE id = $1', [receivable.id]);
                        }
                    }
                    finally {
    ...
    static async addItem(req, res) {
        try {
            ...
            if (item_type === 'product' && product_id) {
                const product = await product_model_1.ProductModel.findById(product_id);
                if (!product) {
            ...
            const item = await order_model_1.OrderModel.addItem({
                order_id: orderId,
            ...
            await order_model_1.OrderModel.updateTotals(orderId);
    ...
    static async updateItem(req, res) {
        try {
            ...
            // Buscar item atual
            const currentItem = await order_model_1.OrderModel.getItemById(itemId);
            ...
            if (currentItem.item_type === 'product' && currentItem.product_id) {
                const product = await product_model_1.ProductModel.findById(currentItem.product_id);
                if (!product) {
                    res.status(404).json({ error: 'Produto não encontrado' });
                    return;
                }
            ...
            // Atualizar item
            const updatedItem = await order_model_1.OrderModel.updateItem(itemId, {
                description,
                quantity,
                unit_price,
    ...
    static async removeItem(req, res) {
        try {
            ...
            const items = await order_model_1.OrderModel.getItems(orderId);
            ...
            const deleted = await order_model_1.OrderModel.removeItem(itemId);
            if (!deleted) {
                res.status(404).json({ error: 'Item não encontrado' });
                return;
            }
    ...
    static async updateDiscount(req, res) {
        try {
            ...
            if (existingOrder.discount !== newDiscount) {
                await order_history_model_1.OrderHistoryModel.create({
                    order_id: id,
            ...
            await order_model_1.OrderModel.update(id, { discount: newDiscount });
            await order_model_1.OrderModel.updateTotals(id);
            const order = await order_model_1.OrderModel.findById(id);
            res.json(order);
        }
        catch (error) {
    ...
    static async generateReceivable(req, res) {
        try {
            const orderId = parseInt(req.params.id);
            if (isNaN(orderId)) {
                res.status(400).json({ error: 'ID da ordem inválido' });
            ...
            try {
                const existingReceivables = await account_receivable_model_1.AccountReceivableModel.findAll(undefined, undefined, undefined, undefined);
                const existingReceivable = existingReceivables.find((ar) => ar.order_id === orderId);
            ...
            const orderWithVehicle = order; // Order pode ter brand/model do JOIN
            ...
            if (use_installments && installment_count && installment_count > 1) {
                try {
                    ...
                    for (let i = 0; i < installmentCount; i++) {
                        ...
                        installmentPromises.push(installment_model_1.InstallmentModel.create({
    ...
    static async getStatistics(req, res) {
        try {
            // Total de OS
            const totalResult = await database_1.default.query('SELECT COUNT(*) as count FROM orders');
            const total = parseInt(totalResult.rows[0].count);
            // Por status
            const statusResult = await database_1.default.query(`SELECT status, COUNT(*) as count FROM orders GROUP BY status`);
            const byStatus = statusResult.rows.reduce((acc, row) => {
                acc[row.status] = parseInt(row.count);
                return acc;
            }, {});
            // Total em valores
            const valueResult = await database_1.default.query(`SELECT 
         ...
         FROM orders`);
            ...
            // OS do mês
            const monthResult = await database_1.default.query(`SELECT COUNT(*) as count, SUM(total) as total
         FROM orders
    ...
    static async quickAction(req, res) {
        try {
            ...
            await order_model_1.OrderModel.update(id, updateData);
            const updatedOrder = await order_model_1.OrderModel.findById(id);
            res.json(updatedOrder);
        }
        catch (error) {
    ...
    static async saveSignature(req, res) {
        try {
            ...
            await order_model_1.OrderModel.updateSignature(id, signature, signed_by_name);
    ...
    static async uploadFile(req, res) {
        try {
            ...
            try {
                ...
                await order_file_model_1.OrderFileModel.ensureTableExists();
                const file = await order_file_model_1.OrderFileModel.create({
                    order_id: id,
                    file_name,
    ...
    static async deleteFile(req, res) {
        try {
            ...
            const file = await order_file_model_1.OrderFileModel.findById(fileId);
            if (!file || file.order_id !== id) {
                res.status(404).json({ error: 'Arquivo não encontrado' });
                return;
            }
            await order_file_model_1.OrderFileModel.delete(fileId);
            res.status(204).send();
        }
        catch (error) {
    ...
    static async createWarranties(req, res) {
        try {
            const orderId = parseInt(req.params.id);
            if (isNaN(orderId)) {
                res.status(400).json({ error: 'ID da ordem inválido' });
...
exports.addItemValidation = [
    ...
    (0, express_validator_1.body)('unit_price').isFloat({ min: 0 }).withMessage('Preço unitário deve ser positivo'),
];
exports.updateItemValidation = [
    ...
    (0, express_validator_1.body)('quantity').optional().isFloat({ min: 0.01 }).withMessage('Quantidade deve ser positiva'),
    (0, express_validator_1.body)('unit_price').optional().isFloat({ min: 0 }).withMessage('Preço unitário deve ser positivo'),
];
exports.saveSignatureValidation = [
    (0, express_validator_1.body)('signature').notEmpty().withMessage('Assinatura é obrigatória'),
    (0, express_validator_1.body)('signed_by_name').notEmpty().withMessage('Nome de quem assinou é obrigatório'),
];
